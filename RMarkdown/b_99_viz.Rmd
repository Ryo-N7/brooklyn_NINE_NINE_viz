...
rating_plot <- ep_rating_df %>% 
  ...
  geom_tile(aes(fill = value), size = 1.5,
            color = "black") +
  scale_fill_gradient(low = "#E6FFFF", high = "#0040FF", 
                       name = "Rating") +
  guides(fill = guide_colourbar(frame.colour = "black")) +
  scale_x_continuous(expand = c(0, 0), 
                     breaks = c(1, seq(from = 5, to = 20, by = 5), 24),
                     labels = c(1, seq(from = 5, to = 20, by = 5), 24)) +
  ...
  theme_minimal() +
  theme(text = element_text(family = "Univers", color = "white"),
        axis.title = element_text(size = 12),
        axis.text = element_text(family = "Univers", color = "white", size = 10),
        legend.title = element_text(family = "Univers", color = "black"),
        legend.text = element_text(family = "Univers", color = "black"),
        legend.position = "bottom",
        legend.key = element_rect(colour = "black", linetype = "solid"),
        legend.background = element_rect(color = "black", fill = "#CBCFD2",
                                         linetype = "solid"),
...
```{r}
# http://id.fnshr.info/2017/03/13/r-plot-tategaki/
tategaki <- function(x){
...
non_main_plot <- non_main_cast %>% 
  ...
  labs(title = "Most appearances by non-main cast", 
       x = NULL, y = tategaki("Number of Episodes")) +
  theme_minimal() +
  theme(text = element_text(family = "Univers", color = "#F9FEFF"),
        axis.text = element_text(size = 8, color = "#F9FEFF"),
...
# session <- bow("https://en.wikipedia.org/wiki/Brooklyn_Nine-Nine_(season_1)")
# 
...
brooklyn99_ep_info_df <- map2(.x = url_wiki_df$urls, .y = url_wiki_df$season_num,
                          ~ brooklyn99_ep_info(url = .x) %>% 
         mutate(season = .y)) %>% 
  reduce(rbind)

saveRDS(brooklyn99_ep_info_df, "../data/brooklyn99_ep_info_df.RDS")
brooklyn_ep_info_df <- readRDS("../data/brooklyn99_ep_info_df.RDS")
brooklyn99_ep_info_df %>% glimpse()
```
...
```{r}
viewers_df <- brooklyn99_ep_info_df %>% 
  ...
  mutate(num_overall = as.numeric(num_overall),
         num_season = as_factor(num_season),
...
cols <- c("#FCF40E", "#3A3533", "#6CA9C3", "#0F0E0E", "#175E78")
# yellow: #FCF40E
# dark grey: #3A3533
# light teal: #6CA9C3
# black: #0F0E0E
...
viewer_plot <- viewers_df %>% 
  ggplot(aes(x = num_overall, y = viewers, group = 1)) +
  ...
  scale_x_continuous(breaks = c(seq(0, 120, by = 5)),
                     labels = c(seq(0, 120, by = 5))) +
  ...
  theme(text = element_text(family = "Univers", color = "#F9FEFF"),
        ...
        panel.grid = element_blank(),
        legend.position = "bottom") +
  annotate(geom = "text", x = 20, y = 15, label = "Aired after Super Bowl!")
