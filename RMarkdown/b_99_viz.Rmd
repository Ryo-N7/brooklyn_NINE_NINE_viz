...
Brooklyn Nine-Nine has become one of my favorite sitcoms in recent years, probably taking over from Parks & Recreation. So in this blog post, I'm going to webscrape some very simple statistics, tidy it up with the `tidyverse`, and visualize it with `ggplot2`. 
...
Another way to visualize this would be the boxplot

this way allows you to make a comparison between seasons whereas the heatmap could be used to pick out what exact episodes in particular had high/low values (although it can be said that you could do this too with a interactive boxplot where you can hover over the individual points and see info).

I can combine the best of both worlds and put the bee swarm points on top of the box plots with the [ggbeeswarm](https://github.com/eclarke/ggbeeswarm) package!
...
ep_rating_df %>% 
  ggplot(aes(season, value, group = season)) +
  geom_boxplot(color = "#F9FEFF", fill = "#000E33") + 
  ggbeeswarm::geom_beeswarm(color = "#FCF40E", cex = 2, size = 2.25) +
...
![the statistical analysis gif]()

well OK they're just box plots Holt is being uncharacteristically melodramatic but I wanted to use thig GIF in some capacity! 
...
![doug judy New york's finest gif]()
...
You can see that it gives you details on what each `dplyr` function changed in your dataframe in the order that the operations were performed. It's pretty cool and it gets more useful as your `dplyr` pipeline grows longer!
