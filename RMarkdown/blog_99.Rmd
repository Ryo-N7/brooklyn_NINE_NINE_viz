...
Brooklyn Nine-Nine has become one of my favorite sitcoms in recent years, probably taking over from Parks & Recreation and Community. So in this blog post, I'm going to web scrape some very simple statistics, clean it up with the `tidyverse`, and visualize it with `ggplot2`... 
...
It is very clear that the Halloween episodes are highly regarded to those that rate B99 on IMDB (although I'm sure most fans including myself will agree).
...
The main cast of B99 are pretty tightly knit and as members of the same precinct it makes sense that they'll generally appear together. So instead, I wanted to look at what non-main cast members and guests made the most appearances on the show. __Special note__: Hitchcock and Scully weren't officially "main cast" until Season 2 but I left them out of the non-main cast list. After scraping for the full cast list and main cast list, I `anti_join()` them so I am left with the non-main cast and the number of episodes that they appeared in.
...
Adrian has appeared in the most episodes just beating out Kevin. This really shows how involved Adrian was in the story in Season 4 and 5 especially compared to Kevin who has been popping in and out since the first season. Everybody's favorite __DOUG JUDY__ rounds off this bar chart.
...
For this plot I use `geom_rect()` to give a colored background for each season. Setting the span of the Xs as the `first` and `last` episode number of each season for the width and using `Inf` and `-Inf` for the Ys to cover the entire height of the plot.
...
The last thing I want to do to wrap this blog post up is to create an infographic containing the plots I made above. Before I combine them all I want to make a title card that looks as similar to the official Brooklyn Nine-Nine title card you see at the beginning of the episode. 
...
Now I could just paste the `URL` for the picture into my graph but why not try making it yourself? So, I used `geom_polygon()` to construct the two diagonal colored bars and then placed the text on top of them with `annotate()`. 
...
After going through this process (read: time-consuming ordeal) I don't recommend making stuff like this manually as it can be quite time consuming especially if you have to get it to fit alongside other stuff!
...
In this blog post I went over creating your own custom `ggplot2` theme, web scraping, cleaning and tidying data frames with the `tidyverse`, and creating a variety of plots to show simple statistics on Brooklyn Nine-Nine. 
